...
public function do_upload_exm(){
	...
	public function studentDetails()
	{
		...
		->where('(admitted_student.left_status = 0 OR admitted_student.long_absent = 0)')
	...
	$this->db->update('admitted_student',$data);
	 ...
	 $adm_stu = $this->db->where('as_id',$left[$i])->row();
	...
	$this->db->update('admitted_student',$data_n);
	 ...
	 $adm_stu = $this->db->where('as_id',$left_n[$i_n])->row();
	 $this->db->where('student_id',$adm_stu->as_student_id);	
	...
	$this->db->update('admitted_student',$data1);
	 ...
	 $adm_stu = $this->db->where('as_id',$absent[$ii])->row();
	 $this->db->where('student_id',$adm_stu->as_student_id);	
	...
	$this->db->update('admitted_student',$data1_n);
	 ...
	 $adm_stu = $this->db->where('as_id',$absent_n[$ii_n])->row();
	 $this->db->where('student_id',$adm_stu->as_student_id);	
	...
	public function hallTicketPDF()
	{
		...
		ob_start();
            ...
            $dompdf->render();
            ob_end_clean();
            $dompdf->stream("HallTicket.pdf", ["Attachment" => 1]);
	...
	public function otherClosingDates()
	{
		...
		$batchyear = $batch_year->batch_from;
		 $data_add = array(
		  ...
		  'sem' => $this->input->post('sem'),
		  'result_publish' => date('Y-m-d',strtotime($this->input->post('result_publish'))),
		  'online_pay_start' => date('Y-m-d',strtotime($this->input->post('online_pay_start'))),
		  'online_pay_end_wo_penalty' => date('Y-m-d',strtotime($this->input->post('online_pay_end_wo_penalty'))),
		  'online_pay_end' => date('Y-m-d',strtotime($this->input->post('online_pay_end'))),
		  'payment_coe' => date('Y-m-d',strtotime($this->input->post('payment_coe'))),
		  'hallticket_issue' => date('Y-m-d',strtotime($this->input->post('hallticket_issue'))),
		  'revaluation_end' => date('Y-m-d',strtotime($this->input->post('revaluation_end'))),
		  ...
		  'result_dspl_date' => date('Y-m-d',strtotime($this->input->post('result_dspl_date'))),
		  'result_dspl_time' => date('H:i:s',strtotime($this->input->post('result_dspl_time'))),
		  'reval_dspl_date' => date('Y-m-d',strtotime($this->input->post('reval_dspl_date'))),
		  'reval_dspl_time' => date('H:i:s',strtotime($this->input->post('reval_dspl_time'))),
		 );
		  if($edit_id != '') {
		$this->db->where('id',$edit_id);
	...
	public function roomAllocationView()
	{
		...
		$data['room_list'] = $rooms_list = $this->db->select('id,block_id,room_id,sem,subject_id,section,batch_year,main_id,course_id,sum(no_of_students) as total_students')->where('batch_year',$batch_year)->where('sem',$sem)->where('room_id',$room_id)->where('section',$section)->group_by('main_id,course_id,subject_id')->from('erp_room_allocation')->get()->result();
	...
	public function roomAllocationAdd()
	{
		...
		array(
                'field' => 'room_id',
                'label' => 'Room',
                'rules' => 'required|is_unique_multiple[erp_room_allocation, batch_year.sem.section.room_id.subject_id,batch.sem.section.room_id.subject]',
				'errors' => array(
                        'required' => 'You must provide a %s.',
    ...
    $rooms = $this->db->select('sum(no_of_students) as tot_no_of_students')->where('section',$section)->where('room_id',$room_id)->where('batch_year',$batch_year)->where('sem',$sem)->get('erp_room_allocation')->row();
    ...
    $rooms = $this->db->select('sum(no_of_students) as tot_no_of_students')->where('id!='.$id.'')->where('section',$section)->where('room_id',$room_id)->where('batch_year',$batch_year)->where('sem',$sem)->get('erp_room_allocation')->row();
	...
	public function seatAllocation()
	{
		...
		$sched = $this->db->where('batch_year',$batch)->where('main_id',$stream)->where('course_id',$department)->where('sem',$sem)->get('erp_exam_schedule')->row();	
	...
	public function attendanceSheet()
	{
		...
		if(isset($_POST['submit'])){
			...
			$data['stream']=$stream = $this->input->post('stream');
			$data['department']=$department = $this->input->post('department');
		$data['room_list'] = $this->db->select('erp_room_allocation.*')
		->where('erp_room_allocation.batch_year',$batch)
		->where('erp_room_allocation.sem',$sem)
		->where('erp_room_allocation.subject_id',$subject)
		->get('erp_room_allocation')->result();
	...
	public function attendanceSheetView()
	{
		...
		$subj = $this->db->where('id',$rooms->subject_id)->get('erp_subjectmaster')->row();
		...
		$data['seat_list'] = $this->db->select('erp_seat_allocation.*')
		->where('erp_seat_allocation.batch_year',$rooms->batch_year)
		->where('erp_seat_allocation.sem',$rooms->sem)
		->where('erp_seat_allocation.subject_id',$rooms->subject_id)
		->where('erp_seat_allocation.block_id',$rooms->block_id)
		->where('erp_seat_allocation.room_id',$rooms->room_id)
		->get('erp_seat_allocation')->result();
		
		
		$data['exam_sched'] = $this->db->select('erp_exam_schedule.*')
		->where('erp_exam_schedule.batch_year',$rooms->batch_year)
		->where('erp_exam_schedule.sem',$rooms->sem)
		->where('erp_exam_schedule.subject_id',$rooms->subject_id)
		->get('erp_exam_schedule')->row();
	...
	public function attendanceSheetPDF()
	{
		...
		$data['subject_id'] = $subject_id = $rooms->subject_id;
		$data['seat_list'] = $this->db->select('erp_seat_allocation.*')
		->where('erp_seat_allocation.batch_year',$rooms->batch_year)
		->where('erp_seat_allocation.sem',$rooms->sem)
		->where('erp_seat_allocation.subject_id',$rooms->subject_id)
...
public function foilSheet(){
  ...
  $data['exam_details'] = '';
	...
	if($sem == 1 ||$sem == 3 ||$sem == 5){
		...
		$data['exam_details'] = 'November -'.date("Y");
