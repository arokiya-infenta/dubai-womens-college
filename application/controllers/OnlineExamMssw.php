...
/* if($this->session->userdata('user_exam')['exam_details'] =="MAHRM"){
    $m = $this->db->select('*')->from('online_exam_question_bank_answe')->where('main_course',$this->session->userdata('user_exam')['exam_details'])->limit(50)->order_by('q_id', $order)->where_in('q_id', ['361','362','363','364','365','366','367','368','369','370','371','372','373','374','375','376','377','378','379','380','381','382','383','384','385','431','432','433','434','435','501','502','503','504','505','506','507','508','509','510','531','532','533','534','535','536','537','538','539','540'])->get();
...
public function Shortlisted(){
	...
	$shortlisted = 	$this->db->select("*")->from("shotlisted_candidate")
	->join(
		"department_details",
		"shotlisted_candidate.sl_main_id=department_details.main_id AND shotlisted_candidate.sl_course_id=department_details.cour_id"
	)
	->where("sl_student_id",$this->session->userdata('user')['user_id'])->where_in("reservation_status",array(1,2,3))->get();
					  
					  
	
	$data['result'] = $shortlisted->result();
		$this->load->view('Home/template/head');
		$this->load->view('Home/site/sortlisted',$data);
		$this->load->view('Home/template/footer');  
