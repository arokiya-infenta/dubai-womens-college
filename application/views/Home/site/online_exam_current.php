...
<div id="online_exm">
    <div class="container">
        <div class="row">
          ...
          </div><div class="col-md-4 clr-changee">Exam End Time : <span class="badge badge-warning p-2" id="eto"><?php echo $et= date('G:i:s',strtotime( $this->session->userdata('user_exam')['exam_time_end']));
       /*   // echo date('h:i:s');
          $date = new DateTime(date('h:i:s'));
          echo "<br>".$date->format('h:i:s');
       /*    $endTime = strtotime("+60 minutes", strtotime($et));
echo date('h:i:s A', $endTime); */  ?></span></div>
...
<form method="post" id="online_exam" action="<?=base_url()?>OnlineExamMssw/postAnswers">

<input type="hidden" name="subject" value="<?=$this->session->userdata('user_exam')['exam_details']?>"/>
...
<input type="hidden" name="controller" id="controller" value="<?php echo $this->uri->segment(1) ;?>"/>
             ...
             <?php  $i=1; foreach($myexam as $exam){  
               ...
               $checked = $this->db->select("*")->from("new_online_exam_answer")->where("user_id",$this->session->userdata('user_exam')['exam_user'])->where("q_id",$exam->q_id)->get();
...
<?php $i++; } ?>
         ...
         <br />
          <div class="pre">
         <input class="btn btn-primary" type="button" id="btnPrevious" value="Previous" style = "display:none"/>
               </div>
                <div class="nxt">
         <input class="btn btn-info" type="button" id="btnNext" value="Next" />
       </div>
...
recorder.stopRecording(function() {
 ...
 //   document.getElementById('header').innerHTML = 'Uploading to PHP using jQuery.... file size: (' +  bytesToSize(fileObject.size) + ')';
    ...
    var upload_url = '<?=base_url()?>/OnlineExamMssw/saveVideo/';
// var upload_url = 'RecordRTC-to-PHP/save.php';

var upload_directory = '<?=base_url()?>/onlineExamVideo/';
