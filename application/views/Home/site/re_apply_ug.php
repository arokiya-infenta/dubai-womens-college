<div class="section layout_padding padding_top padding_bottom-0">
    ...
    <form method="post" id="formId" action="<?=base_url()?>Home/reapplYPaymentUg" enctype="multipart/form-data">
...
$dn = $this->db->select("applied_course_id")->from("Applyed_Cources")->where("user_id",$this->session->userdata('user')['user_id'])->where("main_course_id",5)->get();
...
<input type="checkbox" class="checkbox_class_here"  <?php if(in_array($value->cs_id, $arrt)){ echo "checked disabled";}  ?>  name="course_one[]" value="<?=$value->cs_id?>">
