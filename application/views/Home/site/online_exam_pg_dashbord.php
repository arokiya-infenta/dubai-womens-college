    <div class="section layout_padding padding_bottom-0" style="background:#12385b;padding-top: 79px;">
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <div class="full">
                    <div class="heading_main text_align_center">
                    <h2 class="ttl text-center">Main Exam Dashboard</h2>
                    </div>
                  </div>
            </div>
          </div>
        ...
        <div class="row">
                ...
                <div class="col-md-12 ">
                    ...
                    <!-- <div class="headings">
                     ...
                     <h2 class="ttl">Main Exam Dashboard </h2>
...
$pr_sfcourse = $this->db->select('cour_id,exam_active')->from('department_details')->where('exam_active',1)->where('main_id',1)->get();
$pr_mswsfcourse = $this->db->select('cour_id,exam_active')->from('department_details')->where('exam_active',1)->where('main_id',3)->get();
$pr_mswadcourse = $this->db->select('cour_id,exam_active')->from('department_details')->where('exam_active',1)->where('main_id',2)->get();
...
$exam_code_active = array_values($exam_code_active); 
    ...
    ?>
      ...
      <a href="<?=base_url()?>OnlineExamMssw/myExam/<?=$exam->exam_category?>" class="btn btn-primary bg-clr" ><?=$name?></a>
...
document.getElementById("time_d").innerHTML = "<h2 class='ttl'>Your machine time and the server time does not match.</h2>"+
"<h2 class='ttl'>Server time is " + server_time +"</h2>"+
"<h2 class='ttl'>Log Out now and Set your machine time as 'AUTOMATIC'.</h2>"+
"<h2 class='ttl'>Login Again and take the Main exam.</h2>"+
"<h2 class='ttl'>If you are still unable to set yout time, try changing your device.</h2>";
