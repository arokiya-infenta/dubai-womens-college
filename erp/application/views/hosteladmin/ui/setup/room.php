        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-lg-12">
                            <form name="fees" action="<?=base_url()?>hosteladminlogin/listRoom"  accept-charset="utf-8" method="post" enctype="multipart/form-data">
...
<div class="table-responsive">
 <table id="roomList" class="table table-striped table-bordered table-hover">
  ...
  <tr>
                                    ...
                                    <tbody>
      <?php    foreach($room_list as $row ) { $isData="1";
	  $block=$this->db->where('blockId',$row->blockId)->get('blocks')->row();
	  ?>
                <tr>
                <td><?=$row->roomNo?></td>
                <td><?=$row->noOfSeat?></td>
				...
				<td><a title='Edit' class='btn btn-success btn-circle' href='<?=base_url()."hosteladminlogin/updateRoom/".$row->roomId?>'><i class='fa fa-pencil'></i></a>&nbsp&nbsp<a title='Delete' class='btn btn-danger btn-circle' href='<?=base_url()."hosteladminlogin/deleteRoom/".$row->roomId?>' onclick="return confirm('Are you sure to delete?');"><i class='fa fa-trash-o'></i></a></td>
                </tr>
   <?php } ?>
    </tbody>
                                </table>
		...
		<?php } else
                             {
                                 echo "<h1 class='text-warning'>Room Details Not Found!!!</h1>";
                             }
							 ?>
                        </div>
                    </div>
