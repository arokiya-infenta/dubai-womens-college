...
<?php } ?>
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header titlehms"><i class="fa fa-hand-o-right"></i>Student Meals Report</h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <i class="fa fa-info-circle fa-fw"></i> Meals Information
                    </div>
                    <!-- /.panel-heading -->
			...
			<input type="hidden" value="<?=$deptname?>" id="dept">
				 ...
				 <div class="col-lg-3">
			<input type="month" class="form-control" value="<?=$date1?>" name="date" id="date">
		         </div>
				 <div class="col-lg-12">
...
<div class="table-responsive">
 <table id="mealrate-report" class="table table-striped table-bordered table-hover">
  <thead>
  <tr>
                        <th>S.No </th>
                        <th>Name</th>
      ...
      <?php $sno=1;
					foreach ($stu_list as $student) {
						$get_stu = $this->db->select('attendence.isAbsence')
		->where('attendence.admitId',$student->as_id)
		->where('DATE_FORMAT(attendence.date,"%Y-%m")',$date1)
		->get('attendence')->result();
		           $present = 0; $absent = 0; $rate = 0;
		      foreach ($get_stu as $getstu) { 
						if($getstu->isAbsence==1){
							$present += 1;}
							else{$absent += 1;}
		        }
						$rate = ($present * $mealrate->rate)  +  (($absent * $mealrate->rate)/2);
					?>
                      <tr>
                        <td><?=$sno++?></td>
...
var date_a = '<?=$date1?>';	
 var dept_a = $('#dept').val();		
var myTable = $('#mealrate-report'). DataTable( { 
		"bSortCellsTop": true,
			dom: 'Bfrtip', 
			buttons: 
			[{ 
