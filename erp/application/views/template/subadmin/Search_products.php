...
//search product in invoice
    ...
    public function selectQuantity(){
        ...
        $query =  $this->db->select('*')->from('geopos_units')->get();
        ...
        foreach($res as $val ){
            ...
            $option .=  '<option value="'.$val->code.'">'.$val->name. '-' .$val->code.'</option>';
...

    {
        ...
        if ($name) {
            ...
            foreach ($result as $row) {
                ...
                $name = array($row['product_name'], amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc), $row['pid'], amountFormat_general($row['taxrate']), amountFormat_general($row['disrate']), $row['product_des'], $row['unit'], $row['product_code'], amountFormat_general($row['p_weight']), amountFormat_general($row['weight_qty']), amountFormat_general($row['qty']), $row_num);
...

    {
        ...
        if ($wid > 0) {
            ...
            $qw = "(geopos_products.company_id='$wid') AND ";
        ...
        if ($name) {
            ...
            $query = $this->db->query("SELECT geopos_products.pid,geopos_products.product_name,geopos_products.company_id,

                geopos_products.product_price,geopos_products.product_code,geopos_products.taxrate,geopos_products.disrate,geopos_products.product_des,geopos_products.qty,geopos_products.unit  FROM geopos_products $join WHERE " . $qw . "(UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%') LIMIT 6");
            ...
            foreach ($result as $row) {
                ...
                $name = array($row['product_name'], amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc), $row['pid'], amountFormat_general($row['taxrate']), amountFormat_general($row['disrate']), $row['product_des'], $row['unit'], $row['product_code'], amountFormat_general($row['qty']), $row['company_id'], $row_num);
...

    {
        ...
        if ($name) {
            ...
            $query = $this->db->query("SELECT geopos_products.pid,geopos_products.product_name,geopos_products.product_code,geopos_products.fproduct_price,geopos_products.taxrate,geopos_products.disrate,geopos_products.product_des,geopos_products.unit FROM geopos_products $join WHERE " . $qw . "UPPER(geopos_products.product_name) LIKE '%" . strtoupper($name) . "%' LIMIT 6");
...

    {
        ...
        if ($name) {
            ...
            $query = $this->db->query("SELECT id,sid,name,company,address,city,phone,email FROM geopos_supplier2 WHERE (UPPER(name)  LIKE '%" . strtoupper($name) . "%' OR UPPER(phone)  LIKE '" . strtoupper($name) . "%') LIMIT 6");
            ...
            foreach ($result as $row) {
                ...
                echo "<li onClick=\"selectSupplier('" . $row['id'] . "','" . $row['sid'] . "','" . $row['name'] . " ','" . $row['company'] . "','" . $row['address'] . "','" . $row['city'] . "','" . $row['phone'] . "','" . $row['email'] . "')\"><span>$i</span><p>" . $row['name'] . " &nbsp; " . '<i>' . $row['company'] . '</i>' . " &nbsp; &nbsp  " . $row['phone'] . "</p></li>";
...

    {
        ...
        foreach ($result as $row) {
            ...
            $out .= '    <div class="col-3 border mb-1 "><div class="rounded">
                                 ...
                                 <a   id="posp' . $i . '"  class="select_pos_item btn btn-outline-light-blue round"   data-name="' . $row['product_name'] . '"  data-price="' . amountExchange_s($row['product_price'], 0, $this->aauth->get_user()->loc) . '" data-w1price="' . amountExchange_s($row['wholesale_price'], 0, $this->aauth->get_user()->loc) . '" data-w2price="' . amountExchange_s($row['wholesale_price1'], 0, $this->aauth->get_user()->loc) . '" data-tax="' . amountFormat_general($row['taxrate']) . '"  data-discount="' . amountFormat_general($row['disrate']) . '"   data-pcode="' . $row['product_code'] . '"   data-pid="' . $row['pid'] . '"  data-stock="' . amountFormat_general($row['qty']) . '" data-unit="' . $row['unit'] . '" data-p_weight="' . $row['p_weight'] . '" data-weight_qty="' . $row['weight_qty'] . '" >
