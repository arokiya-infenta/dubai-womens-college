                ...
                <tbody>
					...
					foreach ($stu_list as $student) {
						$subjectt = $this->db->where('id',$subject1)->get('erp_subjectmaster')->row();
		                  ...
		                  // echo  $subjectt1=$get_sub1->id;





						$mark_det = $this->db->where('student_id',$student->id)->where('subject_id',$get_sub1->id)->get('erp_exammark')->row();
						if(isset($mark_det)){
					...
					//	echo $mark_det->ica1Mark ."-". $mark_det->ica2Mark."<br>";	
...
var myTable = $('#exammarks-datatable').DataTable({"search": {regex: true, smart: false}});
