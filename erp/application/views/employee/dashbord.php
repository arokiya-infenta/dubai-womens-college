          ...
          <div class="card border-info border-left-sm">
            <div class="card-body">
              ...
              <div class="media-body text-left">
                <h4 class="text-info">Subjects Taken</h4>
				<?php foreach($subjects as $subject){
					$subname = $this->db->where('id',$subject->subject_id)->get('erp_subjectmaster')->row();
					?>
                  <span class="dash">Batch: <?=$subname->batch_year?> - <?=$subname->subName?> [<?=$subname->subCode?>]</span><br>
				<?php } ?>
              </div>
            </div>
            </div>
              ...
              <div class="media">
               <div class="media-body text-left">
                ...
                <?php foreach($leaves as $leave){
				 ...
				 $count = $count + 1;}}
				 if($count > 0){
					 if($leave->leave_status_principal=='approve'){$stat='<span style="color:green;">Approved</span>';}
					 elseif($leave->leave_status_principal=='reject'){$stat='<span style="color:red;">Rejected</span>';}
