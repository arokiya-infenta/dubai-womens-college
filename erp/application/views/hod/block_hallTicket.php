		  ...
		  <div class="card-header"><i class="fa fa-table"></i>Attendance Percentage Details</div>
			...
			<label>Semester</label>   
			  ...
			  <?php } ?>
				 ...
				 <div class="col-lg-3">
			<label>Stream</label>   
			<select class="form-control" name="stream" id="stream" required disabled>
              ...
              <table id="exammarks-datatable" class="table table-bordered">
                ...
                <tbody>
					...
					?>
						...
						<td>
						<?php $block_stu = $this->db->where('type','HOD')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->get('erp_block_halltickets')->row();
						...
						<?php } ?>
						<?php $block_stu = $this->db->where('type !=','HOD')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->where('status',1)->get('erp_block_halltickets')->result();
						 $types = '';	
