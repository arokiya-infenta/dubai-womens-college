	  ...
	  <!--Start Row-->
	   ...
	   <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
		  <div class="card-header"><i class="fa fa-table"></i>Part V Mark Report</div>
            <div class="card-body">
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              ...
              <table id="partv-report" class="table table-bordered">
                <thead>
                    <tr>
                        <th>S.No </th>
                        <th>Name</th>
                ...
                <tbody>
					...
					foreach ($stu_list as $student) {
						$mark_det = $this->db->where('student_id',$student->id)->where('subject_id',$subject1)->get('erp_partvmark')->row();
...
var base_url = "<?php echo base_url(); ?>";
	$(document).ready(function(){
		...
		$('#batch,#sem').change(function(){
		  ...
		  var sem = $('#sem').val();	
			if (batch!='' && sem!='') {
            $.ajax({
                url: base_url + "hod/getSubjecPartv",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
 ...
 var dept_a = $('#dept').val();	
		var myTable = $('#partv-report'). DataTable( { 
		"bSortCellsTop": true,
			dom: 'Bfrtip', 
			buttons: 
			[{ 
