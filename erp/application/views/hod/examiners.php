	 ...
	 <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
			...
			<div class="row mb-3">
			<div class="col-lg-3">
			<?php if(!isset($appr_stat)){?>
			<a href="<?=base_url().'hod/examinersAdd'?>">
			<button class="btn btn-sm btn-success edit_subject">Add Examiners</button></a>
			<?php } else { ?>
			<span style="font-size:14px; font-weight:bold;">Examiner has been approved</span>
			<?php } ?>
			</div>
			</div>
            </div>
                ...
                <tbody>
					...
					?>
                      <tr>
						...
						<?php if(!isset($appr_stat)){?>
                        <td>
						<a href="<?=base_url(). 'hod/examinersEdit/'.$examiner->id?>">
						<button class="btn btn-sm btn-warning update" data-id="<?=$examiner->id?>" type="button">Edit</button></a>
						<a href="<?=base_url(). 'hod/examinersDelete/'.$examiner->id?>">
