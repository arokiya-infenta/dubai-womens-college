	 ...
	 <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
			...
			<i class="fa fa-table"></i> Edit Examiner
			</div>
            <div class="card-body">
			...
			<option value="<?=$batch->batch_from?>" <?php if($examiner->batch_year==$batch->batch_from){echo 'selected';}?>><?=$batch->batch_from?></option>
			<?php } ?>
			</select>
		         </div>
			  <div class="col-lg-3">
			<label>Semester</label>   
			  ...
			  foreach($sem as $sem){ ?>
			  <option value="<?=$sem?>" <?php if($examiner->sem==$sem){echo 'selected';}?>><?=$sem?></option>
			  <?php } ?>
			  </select>
			  </div>
			...
			<label>Subject</label>   
			  ...
			  <?php $subject = $this->db->where('batch_year',$examiner->batch_year)->where('stream',$stream)->where('department',$department)->get('erp_subjectmaster')->result();
			  foreach($subject as $subject){ ?>
			  <option value="<?=$subject->id?>" <?php if($examiner->subject_id==$subject->id){echo 'selected';}?>><?=$subject->subName?></option>
			  <?php } ?>
			  </select>
			  </div>
			<div class="col-lg-3">
          ...
          <label>Examiner Department</label>
			  ...
			  <select class="form-control" name="department" id="department" value="<?=$dep_val?>" <?=$disabled?>>
			  <?php if($dep_val==0){ ?>
          ...
          <label>Email</label>
			...
			<input type="text" class="form-control" name="college" id="college" value="<?php echo $examiner->college; ?>">
