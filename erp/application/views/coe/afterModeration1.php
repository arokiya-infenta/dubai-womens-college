...
<?php  }else{ ?>
              ...
              <table id="aftermod-datatable" class="table table-bordered">
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					if($stream == 5){
						if($icamrk >= 20 && $ese_m >= 20){$result = '<span data-student="'.$student->id.'" style="color:green;font-weight:bold;">P</span>';}
						else{
							if($icamrk >= 20 && $ese_m < 20){$result = '<span data-student="'.$student->id.'" style="color:red">R(E)</span>';}
							elseif($icamrk < 20 && $ese_m >= 20){$result = '<span data-student="'.$student->id.'" style="color:red">R(I)</span>';}
							else{$result = '<span data-student="'.$student->id.'" style="color:red">R(I + E)</span>';}
								}
					}else{
						if($icamrk >= 25 && $ese_m >= 25){$result = '<span data-student="'.$student->id.'" style="color:green;font-weight:bold;">P</span>';}
						else{
							if($icamrk >= 25 && $ese_m < 25){$result = '<span data-student="'.$student->id.'" style="color:red">R(E)</span>';}
							elseif($icamrk < 25 && $ese_m >= 25){$result = '<span data-student="'.$student->id.'" style="color:red">R(I)</span>';}
							else{$result = '<span data-student="'.$student->id.'" style="color:red">R(I + E)</span>';}
								}
					}
					?>
...
var allPages = myTable.rows().nodes().to$();
    ...
    $('#department').change(function(){
		...
		if(written_year!='' && sem!='' && batch!='' && subject_id!='' && course_id!='' && main_id){
