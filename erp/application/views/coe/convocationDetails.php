        ...
        <div class="col-lg-12">
		  ...
		  <form action="" method="post">
			...
			<?php } ?>
				 ...
				 <div class="col-lg-3 mt-4">
			<button type="submit" class="btn btn-sm btn-success" name="list_submit">Submit</button>
		         </div>
		        </div>
            </form>	
		  </div>
	  <?php if(isset($convocation_list)){
		  ?>
            <div class="card-body">
              ...
              <table id="exammarks-datatable" class="table table-bordered">
                ...
                <tbody>
                    ...
                    <?php $sno=1;
					foreach ($convocation_list as $details) {
						$dept = $this->db->where('main_id',$details->main_id)->where('cour_id',$details->course_id)->get('department_details')->row();
					?>
                      <tr>
                        <td><?=$sno++?></td>
                        <td><?=$batch1?></td>
                        ...
                        <td><button type="button" class="btn btn-danger btn-sm delete" data-id="<?=$details->id?>">Delete</button></td>
                    </tr>
						<?php } ?>
...
var allPages = myTable.rows().nodes().to$();
	...
	$('#stream').change(function(){
		...
		var $ele = $(this).parent().parent();
		 if (confirm('Are you sure to delete the detail?')) {
            $.ajax({
                url: base_url + "coe/deleteConvDet",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
