        ...
        <div class="col-lg-12">
		  ...
		  <div class="card-header">
            ...
            <div class="card-body">
              ...
              <table id="retotal-datatable" class="table table-bordered">
                <thead>
                    <tr>
                        <th>S.No </th>
                        <th>Name</th>
...
var myTable = $('#retotal-datatable').DataTable({ 
		"bSortCellsTop": true,
			dom: 'Bfrtip',
			buttons: 
			[{ 
				extend: 'excelHtml5',
				title: 'Retotalling ' + currentDate + '',
				//text:'<i class="fa fa-table fainfo" aria-hidden="true" > Excel</i>',
                "oSelectorOpts": {filter: 'applied', order: 'current'},
				filename: 'Retotalling_ '+currentDate+'',
...
var allPages = myTable.rows().nodes().to$();
    ...
    $('#department,#sem,#batch').change(function(){
		...
		var retotalling_id=$('#retotalling_id').val();
		...
		var stream=$('#stream').val();
		 if (confirm('Are you sure to update Marks?')) {
            $.ajax({
                url: base_url + "coe/updateRetotalling",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
                    retotalling_id: retotalling_id,solution: solution,status: status,mark_id: mark_id,ica: ica,internal: internal,external: external,thirdparty: thirdparty,average: average,stream: stream,
                },
                success: function (data) {
					$('#myModal .close').click();
                }
