              ...
              <table id="exammarksavgarr-datatable" class="table table-bordered">
                ...
                <tbody> 
					...
					if($av_diff <= 15){
						...
						$average = (float)$mark2;}
						elseif($subject_det->subNature=="PRACTICAL" && $subject_det->msw_m_25==0 ){
						$average = ((float)$mark2 + (float)$mark3) / 2;}
						else{
						$average = ((float)$mark2 + (float)$mark3) / 4;}
						}
...
$average = ((int)$closest + (int)$mark4) / 4;}
					...
					?>
                      <tr>
						...
						<td>
						<input type="text" class="form-control intval" value="<?=$mark2?>" readonly>
						</td>
						<td>
						<input type="text" class="form-control extval" value="<?=$mark3?>" readonly>
						</td>
						<td>
						<input type="text" class="form-control tpval" value="<?=$mark4?>" readonly>
						</td>
						<td>
						<input type="text" class="form-control avval" data-student="<?=$student->id?>" value="<?=$average?>" readonly>
						</td>
                    </tr>
                    <?php } ?>
...
var myTable = $('#exammarksavgarr-datatable').DataTable({ 
		"bSortCellsTop": true,
			...
			[{ 
				extend: 'excelHtml5',
				title: 'Average Mark For Arrear ' + currentDate + '',
				//text:'<i class="fa fa-table fainfo" aria-hidden="true" > Excel</i>',
                "oSelectorOpts": {filter: 'applied', order: 'current'},
				filename: 'Average Mark Arrear_ '+currentDate+'',
