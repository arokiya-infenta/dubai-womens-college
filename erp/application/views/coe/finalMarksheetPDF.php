...
<?php if(sizeof($stu_list) > 0){ ?>
		...
		?>
			...
			<td style="text-transform:uppercase;" colspan=3><?=$specialization?></td>
			<td><?=$month.'-'.date('Y')?></td>
			</tr>
			</table>
<?php } ?>
			...
			<?php //} ?>
			<?php 
			$partv = $this->db->select('s.*,p.status as pstat')->join('erp_subjectmaster s','s.id=p.subject_id')->where('p.batch',$stu_list[0]->batch_year)->where('p.sem',$subj->sem)->where('p.student_id',$student_id)->order_by('(SUBSTRING_INDEX(s.subCode, "/",-1)) asc')->get('erp_partvmark p')->result();
			foreach($partv as $prtv){
				if($prtv->pstat==1){
				 $comp_status = 'COMPLETED'; $parv_res = 'PASS';
				}else{
				$comp_status = 'NOT COMPLETED'; $parv_res = 'RA';}
			?>
			<tr>
			<td><?=$subj->sem?></td>
			<td><?=$prtv->subCode?></td>
			<td style="text-align:left;"><?=$prtv->subName?></td>
			<td><?=ConverToRoman($prtv->part)?></td>
			<td><?=$prtv->creditPnt?></td>
			<td>-</td>
			<td>NA</td>
			<td colspan=3><?=$comp_status?></td>
			<td><?=$parv_res?></td>
			</tr>
			<?php } ?>
			<?php 
		...
		foreach($arrear as $arear){
		 $stulist1 = $this->db->where('student_id',$id)->where('batch_year',$batch)->where('sem',$sem)->where('arrear_status',1)->where('subject_id',$arear->subject_id)->get('erp_exammarkfinal')->row(); 
		 if(isset($stulist1)){	
			...
			if(($subj1->subCode=='MDE/21C/306' || $subj1->subCode=='MS/20C/306') && (($main==1 && $course==15) || $main==2 || $main==3)){$total1=$total1*2;}
			$gp1=$total1*0.1;
			...
			if(($subj1->subCode=='MDE/21C/306' || $subj1->subCode=='MS/20C/306') && (($main==1 && $course==15) || $main==2 || $main==3)){$ica1=$ica1.'/25';$ese1=$ese1.'/25';$total1=($total1/2).'/50';}
		?>
		<tr>
			<td><?=$subj1->sem?></td>
			<td><?=$subj1->subCode?></td>
			<td><?=$subj1->subName?></td>
			<td><?=ConverToRoman($subj1->part)?></td>
		 ...
		 <?php }} ?>
			...
			if(!isset($arrear_history) && ($stu_list[0]->main_id!=5)){
			...
			if($prt_cnt4 != 0){
			if($part4 >= 9.5 && $part4 < 10.0){$grade4='O+';$final_res4='First Class - Exempiary*';}
			if($part4 >= 9.0 && $part4 < 9.5){$grade4='O';$final_res4='First Class - Exempiary*';}
			if($part4 >= 8.5 && $part4 < 9.0){$grade4='D++';$final_res4='First Class - Exempiary*';}
			if(!isset($arrear_history) && ($stu_list[0]->main_id!=5)){
			if($part4 >= 8.0 && $part4 < 8.5){$grade4='D+';$final_res4='First Class with Distinction*';}
			if($part4 >= 7.5 && $part4 < 8.0){$grade4='D';$final_res4='First Class with Distinction*';}
			}else{
			if($part4 >= 8.0 && $part4 < 8.5){$grade4='D+';$final_res4='First Class';}
			if($part4 >= 7.5 && $part4 < 8.0){$grade4='D';$final_res4='First Class';}	
			}
			if($part4 >= 7.0 && $part4 < 7.5){$grade4='A++';$final_res4='First Class';}
			if($part4 >= 6.5 && $part4 < 7.0){$grade4='A+';$final_res4='First Class';}
			if($part4 >= 6.0 && $part4 < 6.5){$grade4='A';$final_res4='First Class';}
			if($part4 >= 5.5 && $part4 < 6.0){$grade4='B+';$final_res4='Second Class';}
			if($part4 >= 5.0 && $part4 < 5.5){$grade4='B';$final_res4='Second Class';}
			if($stu_list[0]->main_id==5){
				if($part4 >= 4.0 && $part4 < 5.0){$grade4='C';$final_res4='Third Class';}
				if($part4 >= 0.0 && $part4 < 4.0){$grade4='U';$final_res4='Re-appear';}
			}else{
				if($part4 >= 0.0 && $part4 < 5.0){$grade4='U';$final_res4='Re-appear';}
			}
			}
			?>
			<tr>
			...
			<td><?=$final_res4?><br\><?php if($part4!=0){echo round($part4,1);}?></td>
			<td>NA</td>
			</tr>
			</table>
			<?php } ?>
