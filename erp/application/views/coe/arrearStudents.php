                ...
                <tbody>
					...
					?>
                      <tr>
						...
						<?php foreach($sem_cnt as $smcnt){ 
						$get_subject = $this->db->select('s.*')->join('erp_subjectmaster s','s.id=em.subject_id')->where('em.batch_year',$batch1)->where('em.sem',$smcnt)->where('em.student_id',$student->id)->where('em.result!="P"')->get('erp_exammarkfinal em')->result();
						$subj = array();
						$get_sub = '';
