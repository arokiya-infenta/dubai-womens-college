	  ...
	  <!--Start Row-->
	   ...
	   <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
		  <div class="card-header"><i class="fa fa-table"></i>Duplicate Consolidated Mark Sheet</div>
            <div class="card-body">
                ...
                <tbody>
					...
					?>
                      <tr>
						...
						<td>
						<a href="<?=base_url().'coe/dupConsolidatedMarksheetPDF/'.$student->id.'/'.$sem1.'/'.$batch1?>">
						<img src="<?=base_url().'system/images/pdf_img.png'?>" style="width:40px;height:40px;"></a>
						</td>
						<td><button class="btn btn-sm btn-success dup_stat" data-id="<?=$student->dup_id?>">Close</button>
						</td>	
                    </tr>
                    <?php } ?>
...
var allPages = myTable.rows().nodes().to$();	
    ...
    $('#department,#stream,#sem,#batch').change(function(){
		...
		$('.dup_stat').click(function(){
		  var id = $(this).data('id');	
		  var ele = $(this).parent();	
		  if(confirm('Are you sure to change the status?')){
            $.ajax({
                url: base_url + "coe/dupMarksheetStat",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
