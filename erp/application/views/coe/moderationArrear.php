          ...
          <div class="card">
            <div class="card-body">
			  ...
			  <?php if($stream == 5){$rng = $this->db->where('main_id','UG')->get('erp_moderationrange')->row();}
						else{$rng = $this->db->where('main_id','UG')->get('erp_moderationrange')->row();}
						if(isset($rng)){
              ...
              <table id="moderationarr-datatable" class="table table-bordered">		
                <thead>
                    <tr>
                        <th>S.No </th>
                        <th>Name</th>
...
var myTable = $('#moderationarr-datatable').DataTable({ 
		"bSortCellsTop": true,
			dom: 'Bfrtip',
			buttons: 
			[{ 
