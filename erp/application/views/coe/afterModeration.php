          ...
          <div class="card">
            <div class="card-body">
			  ...
			  <div class="col-lg-6 mb-4">
				 ...
				 <input type="hidden" name="written_year" value="<?=$year1?>">
...
<table id="aftermod-datatable" class="table table-bordered">
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					if($stream == 5){
						if($icamrk >= 12.5 && $ese_m >= 12.5){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>'; $result_m = 'P';}
						else{
							if($icamrk >= 12.5 && $ese_m < 12.5){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">R(E)</span>'; $result_m = 'R(E)';}
							elseif($icamrk < 12.5 && $ese_m >= 12.5){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">R(I)</span>'; $result_m = 'R(I)';}
							else{$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">R(I + E)</span>'; $result_m = 'R(I + E)';}
								}
					}else{
						if($icamrk >= 12.5 && $ese_m >= 12.5){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>'; $result_m = 'P';}
						else{
					...
					?>
                      <tr>
                        <td><?=$sno++?><input type="hidden" name="studid[]" class="studid" value="<?=$student->id?>" ></td>
                        <td><input  data-student="<?=$student->id?>" type="hidden" name="regnumber[]" class="regnumber" value="<?=$student->reg_no_?>" ><?=$student->reg_no_?></td>
                        <td><input  data-student="<?=$student->id?>" type="hidden" name="student_name_[]" class="student_name_" value="<?=$student->student_name_?>" ><?=$student->student_name_?></td>
						<td align="center">
						<?php if($ica_fin==0){
							$ica_m="A";
							echo "A";}else{
								$ica_m=$ica_fin;
								echo $ica_fin;}?>	
								<input  data-student="<?=$student->id?>" type="hidden" name="ica_mark[]" class="ica_mark" value="<?=$ica_m?>" >
						</td>
						<td align="center"><?php if($ese_m==0){
							$ese_mk ="A";
							echo"A";}else{
								$ese_mk =$ese_m;
								echo $ese_m;}?>
						<input  data-student="<?=$student->id?>" type="hidden" name="ese_mark[]" class="ese_mark" value="<?=$ese_mk?>" >
					</td>
						<td align="center"> 	<input  data-student="<?=$student->id?>" type="hidden" name="tolal[]" class="tolal" value="<?=$total?>" > <?=ceil($total)?></td>
						<td align="center">	<input  data-student="<?=$student->id?>" type="hidden" name="result[]" class="result" value="<?=$result_m?>" >  <?=$result?> </td>
                    </tr>
                    <?php } ?>
...
<?php  }else if($subject[0]->subNature=="PRACTICAL" && $subject[0]->msw_m_25==0){ ?>
	...
	<table id="aftermod-datatable" class="table table-bordered">
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					if($stream == 5){
						if($icamrk >= 20 && $ese_m >= 20){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>'; $result_m = ' P ';}
						else{
							if($icamrk >= 20 && $ese_m < 20){$result = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(E)</span>'; $result_m = ' R(E) ';}
							elseif($icamrk < 20 && $ese_m >= 20){$result = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(I)</span>'; $result_m = ' R(I) ';} 
							else{$result_m = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(I + E)</span>'; $result = 'R(I + E)';}
								}
					}else{
						if($icamrk >= 25 && $ese_m >= 25){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>'; $result_m = ' P ';}
						else{
							if($icamrk >= 25 && $ese_m < 25){$result = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(E)</span>'; $result_m = ' R(E) ';}
							elseif($icamrk < 25 && $ese_m >= 25){$result = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(I)</span>'; $result_m = ' R(I)';}
							else{$result = '<span style="color:red;font-weight:bold;" data-student="'.$student->id.'">R(I + E)</span>'; $result_m = ' R(I + E) ';}
								}
					}
					?>
                      <tr>
											<td><?=$sno++?><input type="hidden" name="studid[]" class="studid" value="<?=$student->id?>" ></td>
                        <td><input type="hidden" name="regnumber[]" class="regnumber" value="<?=$student->reg_no_?>" ><?=$student->reg_no_?></td>
                        <td><input type="hidden" name="student_name_[]" class="student_name_" value="<?=$student->student_name_?>" ><?=$student->student_name_?></td>
						<td align="center">
						<?php if($ica_fin==0){
							$ica_m="A";
							echo "A";}else{
								$ica_m=$ica_fin;
								echo $ica_fin;}?>	
								<input type="hidden" name="ica_mark[]" class="ica_mark" value="<?=$ica_m?>" >
						</td>
						<td align="center"><?php if($ese_m==0){
							$ese_mk ="A";
							echo"A";}else{
								$ese_mk =$ese_m;
								echo $ese_m;}?>
						<input type="hidden" name="ese_mark[]" class="ese_mark" value="<?=$ese_mk?>" >
					</td>
						<td align="center"> 	<input type="hidden" name="tolal[]" class="tolal" value="<?=$total?>" > <?=ceil($total)?></td>
						<td align="center">	<input type="hidden" name="result[]" class="result" value="<?=$result_m?>" >  <?=$result?></td>
                    </tr>
                    <?php } ?>
...
<?php  }else{ ?>
              ...
              <table id="aftermod-datatable" class="table table-bordered">
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					$result_m = '';
					if($stream == 5){
						if($icamrk >= 20 && $ese_m >= 20){
							$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>';
							$result_m = 'P';
						
						}
						else{
							if($icamrk >= 20 && $ese_m < 20){
								$result = '<span style="color:red" data-student="'.$student->id.'">R(E)</span>';
								$result_m = 'R(E)';
							}
							elseif($icamrk < 20 && $ese_m >= 20){
								
								$result = '<span style="color:red" data-student="'.$student->id.'">R(I)</span>';
								$result_m = 'R(I)';
							
							}
							else{
								$result = '<span style="color:red" data-student="'.$student->id.'">R(I + E)</span>';
								$result_m = 'R(I + E)';
							}
								}
					}else{
						if($icamrk >= 25 && $ese_m >= 25){$result = '<span style="color:green;font-weight:bold;" data-student="'.$student->id.'">P</span>';	$result_m = 'P';}
							...
							if($icamrk >= 25 && $ese_m < 25){$result = '<span style="color:red" data-student="'.$student->id.'">R(E)</span>';	$result_m = 'R(E)';}
							elseif($icamrk < 25 && $ese_m >= 25){$result = '<span style="color:red" data-student="'.$student->id.'">R(I)</span>';	$result_m = 'R(I)';}
							else{$result = '<span style="color:red" data-student="'.$student->id.'">R(I + E)</span>';$result_m = 'R(I + E)';}
								}
					}
					?>
                      <tr>
...
$('#publish_result').click(function(){
	...
	if (confirm('Are you sure to save the result?')) {
            $.ajax({
                url: base_url + "coe/SaveMarkResult",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
									...
									student_name_: student_name_,
										...
										tolal: tolal,
										result: result
										,main_id: main_id,course_id: course_id,subject_id: subject_id,batch: batch,sem: sem,written_year: written_year,
                },
                success: function (data) {
