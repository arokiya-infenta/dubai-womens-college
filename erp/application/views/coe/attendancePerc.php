		...
		<?php $this->session->set_flashdata('form_err',''); ?>
		  ...
		  <div class="card-header"><i class="fa fa-table"></i>Attendance Percentage Details</div>
			...
			<label>Type</label>   
			  ...
			  <?php $type = array('1'=>'Condonation','2'=>'Not Eligible');
			  foreach($type as $key => $value){ ?>
			  <option value="<?=$key?>" <?php if($type1==$key){echo 'selected';}?>><?=$value?></option>
			  <?php } ?>
			  </select>
			  </div>
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
				 <div align="middle">
				 <?php if($type1==1){$typ = 'Condonation';}elseif($type1==2){$typ = 'Not Eligible';}else{$typ = 'Redo';}?>
			<h5 style="font-weight:bold;text-transform:uppercase"><?=$typ?></h5>
		         </div>
			</div>
            <div class="card-body">
              ...
              <table id="exammarks-datatable" class="table table-bordered">
                ...
                <tbody>
					...
					foreach ($stu_list as $student) {
						$prec = '';
						$get = $this->db->where('student_id',$student->id)->where('subject_id',$subject1)->where('batch_year',$batch1)->where('main_id',$stream)->where('course_id',$department)->get('erp_stu_attendance')->result();
						$total = $this->db->where('student_id',$student->id)->where('subject_id',$subject1)->where('batch_year',$batch1)->where('main_id',$stream)->where('course_id',$department)->get('erp_stu_attendance')->num_rows();
						if(sizeof($get) > 0){
						$present = $this->db->where('student_id',$student->id)->where('subject_id',$subject1)->where('batch_year',$batch1)->where('main_id',$stream)->where('course_id',$department)->where('attndnce_status',1)->get('erp_stu_attendance')->num_rows();
                    ...
                    <?php }} elseif($type1 == 2){if($prec < 65){ ?>
					<tr>
                        <td><?=$sno++?></td>
                        <td><?=$student->student_name_?></td>
						<td><?=$student->reg_no_?></td>
						<td>
						<?php $not_elig = $this->db->where('batch_year',$batch1)->where('sem',$sem1)->where('subject_id',$subject1)->where('student_id',$student->id)->get('erp_not_eligible')->row();
						if(isset($not_elig) && $not_elig->status == 1){?>
						<p style="font-weight:bold;">Marked Eligible</p>
						<?php } else { ?>
						<button data-student="<?=$student->id?>" class="btn btn-sm btn-warning eligible" id="eligible">Mark Eligible</button>
						<?php } ?>
						</td>
                    </tr>
					<?php }}}} ?>
...
var allPages = myTable.rows().nodes().to$();	
    ...
    $('#department,#stream,#sem,#batch').change(function(){
		...
		$('.eligible',allPages).click(function(){
		  var stream = '<?=$stream?>';	
		  ...
		  var student = $(this).data('student');
			if(confirm('Are you sure to mark the student eligible?')){
			if (stream!='' && department!='' && batch!='' && sem!='') {
            $.ajax({
                url: base_url + "coe/markEligible",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
                ...
                success: function (data) {
					alert('Student has been marked eligible!!');
					location.reload();
                }
            });
			}
