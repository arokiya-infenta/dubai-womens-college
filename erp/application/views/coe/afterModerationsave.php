        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
			  ...
			  <a class="btn btn-sm btn-success" href="<?=base_url()?>Coe/viewReport/<?=$batch1?>/<?=$sem1?>/<?=$subject1?>/<?=$stream?>/<?=$department?>" >View Record</a>
	  </div>
				 </form>
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					?>
                      <tr>
						...
						<td align="center">	<input  data-student="<?=$student->id?>" type="hidden" name="result[]" class="result" value="<?=$result?>" >  <?=$result?> </td>
                    </tr>
                    <?php } ?>
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					?>
                      <tr>
						...
						<td align="center">	<input type="hidden" name="result[]" class="result" value="<?=$result?>" >  <?=$result?></td>
                    </tr>
                    <?php } ?>
				...
				//	if($mark != 0){$ese_fin = $mark;}else{$ese_fin = 0;}
					...
					if($stream == 5){
						if($icamrk >= 20 && $ese_m >= 20){
							$result = '<span style="color:green;font-weight:bold;">P</span>';
							$result_m = 'P';
						
						}
						else{
							if($icamrk >= 20 && $ese_m < 20){
								$result = '<span style="color:red">R(E)</span>';
								$result_m = 'R(E)';
							}
							elseif($icamrk < 20 && $ese_m >= 20){
								
								$result = '<span style="color:red">R(I)</span>';
								$result_m = 'R(I)';
							
							}
							else{
								$result = '<span style="color:red">R(I + E)</span>';
								$result_m = 'R(I + E)';
							}
								}
					}else{
						if($icamrk >= 25 && $ese_m >= 25){$result = '<span style="color:green;font-weight:bold;">P</span>';	$result_m = 'P';}
						else{
							if($icamrk >= 25 && $ese_m < 25){$result = '<span style="color:red">R(E)</span>';	$result_m = 'R(E)';}
							elseif($icamrk < 25 && $ese_m >= 25){$result = '<span style="color:red">R(I)</span>';	$result_m = 'R(I)';}
							else{$result = '<span style="color:red">R(I + E)</span>';$result_m = 'R(I + E)';}
								}
					}
					?>
                      <tr>
...
alert(result);
		...
		$('#publish_result').click(function(e){
			e.preventDefault();
