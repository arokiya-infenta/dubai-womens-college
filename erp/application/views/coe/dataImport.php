...
$('#publish_result').click(function(){
		...
		if(applied_sem == false){ applied_sem='';}	
			...
			$(this).closest('tr').find('td:eq(3) .ica').each(function(){
            student.push($(this).data('student'));
            ica.push($(this).val());
            });
		});
		allPages.each(function(){
			$(this).closest('tr').find('td:eq(4) .ese').each(function(){
            ese.push($(this).val());
            });
		});
		allPages.each(function(){
			$(this).closest('tr').find('td:eq(5) .total').each(function(){
            total.push($(this).val());
            });
		});
		allPages.each(function(){
		...
		if (confirm('Are you sure to save the result?')) {
            $.ajax({
                url: base_url + "coe/saveManualMark",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
                    student: student,ica: ica,ese: ese, total: total, result: result, main_id: main_id,course_id: course_id,subject_id: subject_id,batch: batch,sem: sem,written_year: written_year,
                },
                success: function (data) {
                }
            });
