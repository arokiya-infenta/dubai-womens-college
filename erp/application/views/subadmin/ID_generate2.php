	 ...
	 <div class="row">
        <div class="col-lg-12">
          <div class="card">
			...
			<a href="<?=base_url().'subadmin/IDGenerateDip2'?>"><button class="btn btn-sm btn-success">ID For Diploma</button></a>
			</div>
			</div>
            <div class="card-body">
      ...
      <div class="row">
          ...
          <div class="card">
			...
			<div class="row">
			  <div class="col-lg-12">
				...
				<button id="idgenerate2" class="btn btn-sm btn-success" name="gen_id">Generate ID</button>
				</div>
			  </div>
			</div>
			</div>
		...
		if($department==1){
		$stu_det = $this->db->where('pr_user_id',$stulist->as_student_id)->get('new_preview')->row();
		$stu_mob = $this->db->where('u_id',$stulist->as_student_id)->get('stu_user')->row();	
		}elseif($department==2){
		$stu_det = $this->db->where('pr_user_id',$stulist->as_student_id)->get('new_preview_pg')->row();
        $stu_mob = $this->db->where('u_id',$stulist->as_student_id)->get('stu_user')->row();	
		}else{
		$stu_det = $this->db->where('pr_user_id',$stulist->as_student_id)->get('new_preview_dip')->row();	
        $stu_mob = $this->db->where('u_id',$stulist->as_student_id)->get('stu_user')->row();	
		}
		
		$batch=explode('-',$stulist->as_app_number);
		$doj='01-01-20'.$batch[1]; $deg_p=date('Y',strtotime($doj));
        if($department==1){$deg_period=date("Y", strtotime(date("Y-m-d", strtotime($doj)) . " + 3 year"));} 
        else{$deg_period=date("Y", strtotime(date("Y-m-d", strtotime($doj)) . " + 2 year"));}
		
		$stu_batch = $deg_p.'-'.$deg_period;
					 ?>
                      <tr>
                        <td><?=$sno++?></td>
                        ...
                        <td><?=$stu_batch?></td>
                        <td><?=$stulist->as_dep?></td>
                        <td><?=$stulist->as_blood_gp?></td>
                        <td><?=$stu_mob->u_mobile?></td>
