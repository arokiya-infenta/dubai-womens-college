	  ...
	  <!--Start Row-->
	   ...
	   <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
		  <div class="card-header"><i class="fa fa-table"></i> Block HallTicket</div>
            <div class="card-body">
              ...
              <table id="exammarks-datatable" class="table table-bordered">
                ...
                <tbody>
					...
					?>
						...
						<td>
						<?php $block_stu = $this->db->where('type','LIBRARY')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->get('erp_block_halltickets')->row();
						...
						<?php } ?>
						<?php $block_stu = $this->db->where('type !=','LIBRARY')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->where('status',1)->get('erp_block_halltickets')->result();
						 $types = '';	
...
var allPages = myTable.rows().nodes().to$();
	...
	$('#stream').change(function(){
		  ...
		  var stream = $(this).val();	
			if (stream!='') {
            $.ajax({
                url: base_url + "librarylogin/getProgram",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
    ...
    $('#department').change(function(){
		  ...
		  var batch = $('#batch').val();	
            ...
            $.ajax({
                url: base_url + "librarylogin/getSubj",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
		...
		$('.block',allPages).click(function(){
		  ...
		  if(status==0){var bl='Unblock';}else{var bl='Block';}
            ...
            $.ajax({
                url: base_url + "librarylogin/blockHt",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
