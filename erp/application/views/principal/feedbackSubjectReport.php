		...
		<?php $this->session->set_flashdata('form_err',''); ?>
		  ...
		  <div class="card-header"><i class="fa fa-table"></i>Feedback Subject Report</div>
			...
			<label>Subject</label>   
			  ...
			  <?php if($subject1 != ''){$sub1 = $this->db->where('id',$subject1)->get('erp_subjectmaster')->row();  
			  $employee = $this->db->select('em.emp_name_ as name')->join('erp_employee_master em','em.id=es.employee_id')->where('subject_id',$subject1)->get('erp_employee_subject es')->row();
			  $emp_name = '';
				if(isset($employee)) $emp_name = $employee->name;
			  ?>
			<input type="hidden" value="<?=$sub1->subName?>" id="sub_name">
			<input type="hidden" value="<?=$sub1->subCode?>" id="sub_code">
			<input type="hidden" value="<?=$emp_name?>" id="emp_name">
			<?php } ?>
			  </div>
        ...
        <div class="col-lg-12">
		  ...
		  <div class="card-header">
            ...
            <div class="card-body">
              ...
              <table id="feedback-subject" class="table table-bordered">
                <thead>
                    <tr>
                        <th>S.No </th>
                        <th>Questions</th>
                ...
                <tbody>
					...
					foreach ($feedbacks as $feedbacks) {
						...
						$fbtot = 0;
						$avg = 0;
						$fb_sum = $this->db->select('SUM(fr_'.$sno.') as sum')->where('fr_subject_id',$subject1)->get('feedback_course_report')->row();
						$fbtot = $this->db->where('fr_subject_id',$subject1)->get('feedback_course_report')->num_rows();
						$fbsum = $fb_sum->sum;
						if($fbtot!=0){
						$avg = $fbsum / $fbtot;}
					?>
		...
		"bSortCellsTop": true,
			...
			[{ 
				...
				customize: function (doc) {
				   ...
				   doc.content[1].table.widths = [ '5%', '60%', '10%'];
				   var rowCount = doc.content[1].table.body.length;
					...
					doc.content.splice( 1, 1, {
                        ...
                        bold: true,
                        fontSize: 10,
                        text: 'Student Feedback on Teacher',
						margin: [0, 0, 0, 0],
                    } );
					...
					doc.content.splice(4, 0, {
                        ...
                        bold: true,
                        fontSize: 10,
                        text: [ {text: ''+emp_name+'-'+sub_code+'-'+sub_name+'', bold: true, fontSize: 10, margin: 30} ],
                    });
