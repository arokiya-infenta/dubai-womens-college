              ...
              <table id="exammarks-datatable" class="table table-bordered">
                ...
                <tbody>
					...
					?>
						...
						<td>
						<?php $block_stu = $this->db->where('type','PRINCIPAL')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->get('erp_block_halltickets')->row();
						...
						<?php } ?>
						<?php $block_stu = $this->db->where('type !=','PRINCIPAL')->where('batch_year',$batch1)->where('sem',$sem1)->where('student_id',$student->id)->where('status',1)->get('erp_block_halltickets')->result();
						 $types = '';	
    ...
    /*$('#department').change(function(){
		...
		$('.block',allPages).click(function(){
		  ...
		  if(status==0){var bl='Unblock';}else{var bl='Block';}
            ...
            $.ajax({
                url: base_url + "principal/blockHt",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
