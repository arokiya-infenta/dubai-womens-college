	  ...
	  <!--Start Row-->
	   ...
	   <div class="row">
		...
		<?php $this->session->set_flashdata('form_err',''); ?>
          <div class="card">
		  <div class="card-header"><i class="fa fa-table"></i>Teacher Feedback Report</div>
            <div class="card-body">
        ...
        <div class="col-lg-12">
		  ...
		  <div class="card-header">
            ...
            <div class="card-body">
              ...
              <table id="feedback-status" class="table table-bordered">
                <thead>
                    <tr>
                        <th>S.No </th>
                        <th>Teacher Name</th>
                ...
                <tbody>
                    ...
                    <?php if(sizeof($subject1) > 0){ $sno=1; 
					foreach ($subject1 as $subject) {
						$employee = $this->db->where('subject_id',$subject->id)->get('erp_employee_subject')->result();
						$name = '';
						$empname = array();
						foreach($employee as $employee){
						$emp_name = $this->db->where('id',$employee->employee_id)->get('erp_employee_master')->row();
						array_push($empname, $emp_name->emp_name_);
						}
						$name = implode(',',$empname);
						$fbsum = 0;
						$fbtot = 0;
						$avg = 0;
						$fb_sum = $this->db->select('SUM(fr_1 + fr_2 + fr_3 + fr_4 + fr_5 + fr_6 + fr_7 + fr_8 + fr_9 + fr_10 + fr_11 + fr_12) as sum')->where('fr_subject_id',$subject->id)->get('feedback_course_report')->row();
						$fbtot = $this->db->where('fr_subject_id',$subject->id)->group_by('fr_id')->get('feedback_course_report')->num_rows();
						$fbsum = $fb_sum->sum;
						if($fbtot!=0){
						$avg = $fbsum / $fbtot;}
					?>
                      <tr>
                        <td><?=$sno++?></td>
                        <td><?=$name?></td>
                        <td><?=$subject->subName?> <br> <?=$subject->subCode?></td>
                        <td><?=$avg?></td>
                    </tr>
					<?php }} ?>
		...
		"bSortCellsTop": true,
			...
			[{ 
				...
				customize: function (doc) {
				   ...
				   doc.content[1].table.widths = [ '5%', '14%', '30%', '30%'];
				   var rowCount = doc.content[1].table.body.length;
					...
					doc.content.splice( 1, 1, {
                        ...
                        bold: true,
                        fontSize: 10,
                        text: 'Feedback Report on Teachers',
						margin: [0, 0, 0, 0],
                    } );
                    ...
                    doc.content.splice(2, 0, {
                        ...
                        bold: true,
                        fontSize: 10,
                        text: [ {text: ''+dept_f+'', bold: true, fontSize: 10, margin: 30} ],
                    });
