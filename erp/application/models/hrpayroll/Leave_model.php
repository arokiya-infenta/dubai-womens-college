<?php
    ...
    public function GetEarneBalanceByEmCode($id){
        ...
        `employee_hr`.`em_id`,CONCAT(`first_name`, ' ', `last_name`) AS emname
        FROM `earned_leave`
        LEFT JOIN `employee_hr` ON `earned_leave`.`em_id`=`employee_hr`.`em_id`
    ...
    public function GetEmLEaveReport($emid, $day, $year){
        ...
        $sql = "SELECT `emp_leave`.*,
                (SELECT SUM(`leave_duration`) 
                    ...
                    `employee_hr`.`first_name`,`last_name`,`em_code`,
                    `leave_types`.`name`
                FROM `emp_leave`
                    LEFT JOIN `leave_types` ON `emp_leave`.`typeid`=`leave_types`.`type_id`
                    LEFT JOIN `employee_hr` ON `emp_leave`.`em_id`=`employee_hr`.`em_id`
...
/*public function GetEmLEaveReport($emid, $date){
        ...
        if($emid == "all") {
            $sql = "SELECT `assign_leave`.*,
        `employee_hr`.`first_name`,`last_name`,
        `leave_types`.`name`
        FROM `assign_leave`
        LEFT JOIN `leave_types` ON `assign_leave`.`type_id`=`leave_types`.`type_id`
        LEFT JOIN `employee_hr` ON `assign_leave`.`emp_id`=`employee_hr`.`em_id`
        WHERE `dateyear`='$date'
        ";
    } else {
...
*/
    ...
    public function GetEarnedleaveBalance(){
        $sql = "SELECT `earned_leave`.*, `employee_hr`.`first_name`,`last_name`,`em_code` FROM `earned_leave` LEFT JOIN `employee_hr` ON `earned_leave`.`em_id`=`employee_hr`.`em_id` WHERE `earned_leave`.`hour` > 0 AND `employee_hr`.`status`='ACTIVE'";
        $query = $this->db->query($sql);
        $result = $query->result();
        return $result; 
    }
