<?php
    ...
    $sql = "SELECT * FROM `employee_hr` WHERE `status`='ACTIVE'";
    $query=$this->db->query($sql);
  	$result = $query->result();
  	return $result;
	}
    public function emselectByID($emid){
    $sql = "SELECT * FROM `employee_hr`
      WHERE `em_id`='$emid'";
    $query=$this->db->query($sql);
	$result = $query->row();
	return $result;
    ...
    public function Does_email_exists($email) {
		$user = $this->db->dbprefix('employee_hr');
    ...
    public function Add($data){
        $this->db->insert('employee_hr',$data);
    }
    public function GetBasic($id){
		$user=$this->session->userdata('user')['user_id'];
      $sql = "SELECT `erp_employee_master`.*,
      `erp_dept_type`.*,
      `erp_role_master`.*
      FROM `erp_employee_master`
      LEFT JOIN `erp_dept_type` ON `erp_employee_master`.`emp_dept_type_`=`erp_dept_type`.`id`
      LEFT JOIN `erp_role_master` ON `erp_employee_master`.`emp_designation_`=`erp_role_master`.`id`
      WHERE erp_employee_master.id=".$user."";
        $query=$this->db->query($sql);
		$result = $query->row();
		return $result;          
    }
    public function ProjectEmployee($id){
      ...
      `employee_hr`.`em_id`,`first_name`,`last_name`
      FROM `assign_task`
      LEFT JOIN `employee_hr` ON `assign_task`.`assign_user`=`employee_hr`.`em_id`
    ...
    public function desciplinaryfetch(){
      ...
      `employee_hr`.`em_id`,`first_name`,`last_name`,`em_code`
      FROM `desciplinary`
      LEFT JOIN `employee_hr` ON `desciplinary`.`em_id`=`employee_hr`.`em_id`";
        $query=$this->db->query($sql);
		$result = $query->result();
		return $result;        
    }
    ...
    public function Update($data,$id){
		...
		$this->db->update('employee_hr',$data);        
    }
    ...
    public function GetEmployeeId($id){
        $sql = "SELECT `em_password` FROM `employee_hr` WHERE `em_id`='$id'";
        $query = $this->db->query($sql);
        $result = $query->row();
        return $result; 
    }
