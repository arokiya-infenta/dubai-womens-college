...
class Attendance_model extends CI_Model
{
    ...
    public function em_attendance()
    {
      ...
      FROM `attendance`
      LEFT JOIN `employee_hr` ON `attendance`.`emp_id`=`employee_hr`.`em_code` WHERE `attendance`.`status` = 'A' ORDER BY `attendance`.`id` DESC";
    ...
    public function em_attendanceFor($id)
    {
      ...
      `employee_hr`.`em_id`,`first_name`,`last_name`,`em_code`
      FROM `attendance`
      LEFT JOIN `employee_hr` ON `attendance`.`emp_id`=`employee_hr`.`em_code` 
    ...
    public function getPINFromID($employee_ID) {
      $sql = "SELECT `em_code` FROM `employee_hr`
    ...
    {
      $sql    = "SELECT `attendance`.*,
      `employee_hr`.`em_id`, CONCAT(`first_name`, ' ', `last_name`) AS name,`em_code`, TRUNCATE((ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )))/3600, 1) AS Hours
      FROM `attendance`
      LEFT JOIN `employee_hr` ON `attendance`.`emp_id` = `employee_hr`.`em_code` 
