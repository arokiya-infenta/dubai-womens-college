...
echo $this->session->flashdata('message');
    ...
    <form action="<?=base_url()?>accounts/announcements" method="post" enctype="multipart/form-data">	
	        <div class="row">
			<div class="col-md-3">
			<label>Stream</label>
			...
			<label>Remark</label>
			<textarea type="text"  class="form-control " placeholder="Enter Remarks" required name="remark"></textarea>
			</div>
			</div>
<div class="row">
			<div class="col-md-12">
...
$.ajax({
                url: base_url + "accounts/delete_announcement",
                type: 'POST',
                cache: false,
                data: {
                    delete_id: mnt,
...
$("#main_course_id").change(function () {
	$("#stu").css('display','none');
	  ...
	  $(".remark").val('');
        if ($('#main_course_id').val() != "") {
            $.ajax({
                url: base_url + "accounts/get_app_course_id_fun",
                type: 'POST',
                cache: false,
                data: {
  ...
  if($('#app_course_id').val() != "" && $('#main_course_id').val() != "")	{
	  ...
	  $(".remark").val('');
	  $.ajax({
                url: base_url + "accounts/get_announcement",
                type: 'POST',
                cache: false,
                data: {
                    main_course_id: $('#main_course_id').val(),
