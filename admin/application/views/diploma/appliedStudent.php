...
echo"<p style='color: red'> Not Verified </p>";
                        ...
                        <a   class="btn btn-sm btn-primary" href="<?=base_url()?>PgDiploma/updateStudent/<?=$value->u_id?>"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true"></i></a>
					...
					<!--	<button type="submit" name="submit_pdf" class="btn btn-sm btn-success"><i class="fa fa-file-pdf-o  fa-2x" aria-hidden="true"></i></button>-->
						<a href="<?=base_url()?>PgDiploma/updateStuContactInfo/<?=$value->u_id?>" class="btn btn-sm btn-success"><i class="fa fa-address-book-o  fa-2x" aria-hidden="true"></i></a>
						<a href="<?=base_url()?>PgDiploma/studentPdf/<?=$value->u_id?>" class="btn btn-sm btn-danger"><i class="fa fa-file-pdf-o  fa-2x" aria-hidden="true"></i></a>
            <a href="<?=base_url()?>PgDiploma/studentCertificate/<?=$value->u_id?>" class="btn btn-sm btn-info"><i class="fa fa-comments fa-2x" aria-hidden="true"></i></a>
            ...
            <a href="#" data-toggle="modal" data-target="#submittedStudent<?=$value->u_id?>" class="btn btn-sm btn-secondary"><i class="fa fa-certificate fa-2x" aria-hidden="true"></i></a>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <form method="post" action="<?=base_url()?>PgDiploma/updatePGPer">
...
echo "<br><a target='_blank'  href='".base_url()."PgDiploma/UploadFile?file=".urlencode($value->pr_community_cer)."'>Download Certificate</a>";
...
<div class="modal fade" id="exampleMail<?=$value->u_id?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
    <form method="post" action="<?=base_url()?>PgDiploma/personalMailUser">
      <div class="modal-header">
...
<div class="modal fade" id="submittedStudent<?=$value->u_id?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
    <form method="get" action="<?=base_url()?>PgDiploma/OnlineExamMark">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Assign Exam Mark</h5>

        <?php $q = $this->db->select("*")->from("online_exam_pannel")->where("student_id",$value->u_id)->get(); 
...
$update = $res[0]->e_id;
$question = $res[0]->question_attended;
$mark = $res[0]->total_mark;
...
?>
      ...
      <input type ="hidden" name="student_id" value="<?=$value->u_id?>">
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">Total Mark</label>
            <input type="text" class="form-control" name="total_mark" required value="<?=$question?>">
          </div>
          <div class="form-group">
        <input type ="hidden" name="student_id" value="<?=$value->u_id?>">
  <label class="col-form-label " required for="recipient-name">Assign Mark</label>
  <div class="col-md-12">
 <input  class="form-control" required name="assign_mark" value="<?=$mark?>">
