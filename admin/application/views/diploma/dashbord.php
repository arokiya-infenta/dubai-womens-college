...
//print_r($_SESSION);
$appt = $this->db->select('*')->from('stu_user')->where("u_course",3)->get();
...
$intr = $this->db->select('*')->from('Applyed_Master')->where("main_course_priority","DIP")->get();
...
$alloc = $this->db->select('*')->from('Applyed_Cources')->where("applied_course_id",$this->session->userdata('user')['user_dep_status'])->get();
      ...
      <div class="row">
        <div class="col-lg-6">
          <div class="card">
            ...
            <?php $cou = $this->db->select('*')->from('college_course')->where('mc_id',3)->get(); 
