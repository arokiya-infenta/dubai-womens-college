  ...
  <div class="content-wrapper">
    <div class="container-fluid">
	  ...
	  <?php if(isset($stu_list)){
		  $closing_date = $this->db->where('batch_year',$batch1)->where('main_id',$stream)->where('course_id',$department)->get('erp_ica_marks_closing')->row();
		  if(isset($closing_date)){
	  if(date('Y-m-d') > $closing_date->closing_date){ $avail = 1; } else { $avail = 2; }
		  } else { $avail = 3; }
		  if($avail == 1 || $avail == 3){
			  $coe_stat = $this->db->where('batch',$batch1)->where('subject_id',$subject1)->where('main_id',$stream)->where('course_id',$department)->get('erp_ica_to_coe')->row();
			  if(isset($coe_stat)){ if($coe_stat->status == 0){$coe_avail = 1;}else{$coe_avail = 2;}}else{$coe_avail = 3;}
			  if($coe_avail == 1 || $coe_avail == 3){
		  ?>
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
		  ...
		  <button type="button" class="btn btn-sm btn-warning" id="to_coe" style="float:right">Submit to COE</button>
		  </div>
            <div class="card-body">
		  ...
		  ?>
                ...
                <tbody>
					...
					foreach ($stu_list as $student) {
						$mark_det = $this->db->where('student_id',$student->id)->where('subject_id',$subject1)->get('erp_exammark')->row();
						if(isset($mark_det)){
	  ...
	  <div class="row">
		...
		<p>ICA Marks for the subject has been forwarded to COE.</p>
		</div>
		</div>
		   </div>
	  ...
	  <?php }} else{ ?>
        ...
        <div class="col-lg-12">
		<div class="form-group" align="center">
		<p>ICA Marks has not been closed for the Batch.</p>
		</div>
		</div>
		   </div>
...
$('#batch').change(function(){
		  ...
		  var batch = $(this).val();	
			if (batch!='') {
            $.ajax({
                url: base_url + "pgSelfFinance/getSubj",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
...
$("input[type=checkbox]",allPages).each(function () {
	...
	$(".takehome",allPages).change(function () {
		...
		if (confirm('Are you sure to add the Marks?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/ICA1Mark",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
		...
		if (confirm('Are you sure to add the Marks?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/ICA2Mark",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
		...
		if (confirm('Are you sure to add the Marks?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/inClassMark",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
		...
		if (confirm('Are you sure to add the Marks?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/takeHomeMark",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
		...
		$('#to_coe').click(function(){
		var main_id=$('#stream').val();	
		var course_id=$('#department').val();
        var subject_id=$('#subject').val();	
        var batch=$('#batch').val();	
		if (confirm('Are you sure to forward the Marks to COE?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/submitToCoe",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
                    main_id: main_id,course_id: course_id,subject_id: subject_id,batch: batch,
                },
                success: function (data) {
