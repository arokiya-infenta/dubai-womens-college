...
<?php  $sl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",1)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",1)->where('created >','2022-06-01')->get();
$sl_cou = $sl->num_rows();



$wl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",1)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",2)->where('created >','2022-06-01')->get();
$wl_cou = $wl->num_rows();


$rl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",1)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",3)->where('created >','2022-06-01')->get();
$rl_cou = $rl->num_rows();
...
?>
					 ...
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>PgSelfFinance/viewAllotment/1" >Selection List : <?=$sl_cou?> </a>
           </div>
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>PgSelfFinance/viewAllotment/2" >Waiting List : <?=$wl_cou?></a>
					 </div>
					 	 <div class="col-md-4 ">
              <a href = "<?=base_url()?>PgSelfFinance/viewAllotment/3" >Not Eligible List: <?=$rl_cou?></a>
					 </div>
...
<?php  $sl = $this->db->select("*")->from("resrvation_table")->where("rc_main_id",1)->where("rc_cource_id",$this->session->userdata('user')['user_dep_status'])->get();
$sl_cou = $sl->result();

foreach($sl_cou as $v){

    $snt = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",1)
  
  ->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])
  ->where("selection_list_name",$v->rs_name)
  ->where("reservation_status",1)
	->where('created >','2022-06-01')
  ->get();
  $snt_r= $snt->num_rows(); ?>
        ...
        <div class="col-lg-12">
          <div class="card">
           ...
           <div class="col-md-2">
            ...
            <div class="card-body">
            <form method="post" action="<?=base_url()?>PgSelfFinance/registerAllocation">
