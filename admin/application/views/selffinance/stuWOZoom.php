      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            ...
            <?php   }?><a class="text-right btn btn-primary" href = "<?=base_url()?>PgSelfFinance/panelPdfWOZoom/<?=$panel1?>"><i class="fa fa-file-text" aria-hidden="true"></i> Download Panel Report</a></span>
			</div>
			</div>
			</div>
			   <div class="row">
            ...
            <div class="card-body">
              ...
              <table id="wozoom" class="table table-bordered">
				...
				<?php 
                      $sno = 1;
					  foreach($stu_list as $slist) { 
				$get_stu = $this->db->where("stu_id", $slist->pr_user_id)->where("exam_name", $this->Subject)->where("app_course_id", $dept)->where("main_course_id", "1")->where("type", "panel")->where("panel_id", $panel1)->get("student_complete_mark")->row();
				...
				?>
					...
					$app_no= $this->db->select("*")->from("Applyed_Cources")->Where("user_id",$slist->pr_user_id)->Where("applied_course_id",$dept)->where('main_course_id','1')->get()->row();
...
var allPages = myTable.rows().nodes().to$();
		...
		if (confirm('Are you sure to add the Marks?')) {
            $.ajax({
                url: base_url + "pgSelfFinance/updatePanelMarkWOZoom",
                type: 'POST',
                cache: false,
				//dataType: "html",
                data: {
...
if (r == true) {
    ...
    //alert("yes");
   $.ajax({
			url: "<?php echo base_url();?>pgSelfFinance/publishPanelWOZoom",
			method: "POST",
			cache: false,
			data:{
				panel_id: panel_id,
