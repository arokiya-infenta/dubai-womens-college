      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            ...
            <div class="col-lg-4">    
            <form id="target" action="<?=base_url()?>UgSelfFinance/publishResult">
                  ...
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-header">
                       ...
                       $m =  $this->db->select('*')->from('admitted_student')->where('as_quata',$value->selection_list_name)->where('as_dep',$dep)->where("year",$this->session->userdata("user")["user_aca_year"])->get();
                        
                        $n = $m->num_rows();



                        $s =  $this->db->select('*')->from('admitted_student')->where('as_student_id',$value->sl_student_id)->where('as_reg_num !=',null)->where('as_dep',$dep)->where("year",$this->session->userdata("user")["user_aca_year"])->get();
...
if($t == 1){
  ...
  $st = "";
                      ...
                      <form method="POST" action="<?=base_url()?>UgSelfFinance/admitStudent">
...
<text type="text"  class="form-control" value="<?=$value->pr_permanent_address?>" name="app_address"><?=$value->pr_permanent_address?></text>
                      ...
                      <img src="<?=base_url()?>/uploads/<?=$value->pr_photo?>"  width="200px" height="300px"  ></div>
                      </div>
