...
echo $this->session->flashdata('message');
      ...
      <div class="col-lg-12">
          <div class="card">
            <div class="card-header"><i class="fa fa-file-text"></i> shortlist Status <!--<a  class="anc btn btn-info text-right" href="<?=base_url()?>UgSelfFinance/calculateMark">Calculate Candidates' Total</a>--></div>
            <div class="card-body">
            
            <div class="row">

<?php  $sl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",5)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",1)
...
$wl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",5)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",2)
->where('shotlisted_candidate.created >',$this->syear.'-06-01')
->where('shotlisted_candidate.created <',$this->eyear.'-06-01')->get();
$wl_cou = $wl->num_rows();


$rl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",5)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",3)
->where('shotlisted_candidate.created >',$this->syear.'-06-01')
->where('shotlisted_candidate.created <',$this->eyear.'-06-01')->get();
$rl_cou = $rl->num_rows();
...
?>
					 ...
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>UgSelfFinance/viewAllotment/1" >Selection List : <?=$sl_cou?> </a>
           </div>
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>UgSelfFinance/viewAllotment/2" >Waiting List : <?=$wl_cou?></a>
					 </div>
					 	 <div class="col-md-4 ">
              <a href = "<?=base_url()?>UgSelfFinance/viewAllotment/3" >Not Eligible List: <?=$rl_cou?></a>
					 </div>
...
<?php  $sl = $this->db->select("*")->from("resrvation_table")->where("rc_main_id",5)->where("rc_cource_id",$this->session->userdata('user')['user_dep_status'])
	->get();
$sl_cou = $sl->result();

foreach($sl_cou as $v){

    $snt = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",5)
...
<a href="#"><?=$v->rs_name?> List : <?=$snt_r?></a><br>
...
<a style="padding-right: 0.5em;" href="#"><?=$v->rs_name?> List : <?=$snt_r?></a> <br>
<?php
...
?>
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header"><i class="fa fa-file-text"></i> Shortlist criteria </div>
            <div class="card-body">
    ...
    <option value = "TNHSC" >TN HSC</option>
    <option value = "Matric. HSS" >Matric. HSS</option>
        ...
        <div class="col-lg-12">
          <div class="card">
           ...
           <div class="col-md-2">
            ...
            <div class="card-body">
            <form method="post" action="<?=base_url()?>UgSelfFinance/registerAllocation">
...
saveData.error(function() { alert("Something went wrong"); });
  ...
  $("#submit").click(function(){
    ...
    var myKeyVals = { res : res, count : count, special_res : special_res ,board:board   
    }
