      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            ...
            <div class="col-lg-1"> <form id="target" action="">
            ...
            <div class="col-lg-3">Choose the Upcoming Selection List & then Mark the Candidate as Not Eligible</div>
            <div class="col-lg-3">   
      ...
      ->order_by("selection_list", "DESC")
        ...
        $sel = $user->result();   
           ...
           ?> 
            ...
            <select id="csl" name="csl" class="form-control" >
  <option value="0">Upcoming Selection List</option>
...
<div class="col-lg-2">       
<a href="<?=base_url()?>UgSelfFinance/completeSelectionlistPdf/<?=$this->uri->segment(3)?>"  class="btn btn-danger" > Complete Selection list</a>
          </div>   
          </div>
          </div>
            <div class="card-body">
...
$i=1;
                    ...
                    foreach ($student as $key => $value) { 
                      ...
                      ?>
                        ...
                        <td><?=sprintf ("%.2f",$value->percentage_obtained)?></td>
                        <td><?=$value->pr_community?></td>
                        <td><?php if($value->af_paid_status == 1){
echo"Paid";

												}else{echo"Not Paid";}?></td>
                        <td><?php if($value->af_paid_status == 1){
echo date("d-M-Y",strtotime($value->af_created));
                        ...
                        <td><?=$value->selection_list_name?></td>
												<?php if($value->selection_list == 1 ){ ?>
													...
													<?php }else if($value->selection_list == 5){  ?>
														<td>Fifth</td>
													<?php }else if($value->reservation_status == 6){  ?>
														<td>Six</td>	
														<?php }else if($value->reservation_status == 7){  ?>
														<td>Seven</td>
															<?php }else if($value->reservation_status == 8){  ?>
														<td>Eight</td>
															<?php }else if($value->reservation_status == 9){  ?>
														<td>Nine</td>
														<?php  }else{ ?>
															<td>Ten</td>
															<?php }  ?>
...
return false;
  ...
  $("#allsele_pdf").attr("href", "<?= base_url() ?>UgSelfFinance/SelectionlistPdf/<?=$this->uri->segment(3); ?>/"+csl);
 // return true;
 //$("#allsele_pdf").click();
}
//e.preventDefault();
...
var saveData = $.ajax({
    type: 'POST',
    url: "<?=base_url()?>UgSelfFinance/ManualSelectWaitingList",
    data: {id:m,sel:csl},
    dataType: "text",
...
alert("Choose the Upcoming Selection List & then Mark the Candidate as Not Eligible");
return false;
...
alert(csl); */
  ...
  var saveData = $.ajax({
      type: 'POST',
      url: "<?=base_url()?>UgSelfFinance/SelectWaitingList",
      data: {id:m,sel:csl},
      dataType: "text",
      success: function(resultData) { 
//alert(resultData);
...
if (r == true) {
  ...
  var saveData = $.ajax({
      type: 'POST',
      url: "<?=base_url()?>UgSelfFinance/rejectSelectionList",
      data: {id:m},
      dataType: "text",
      success: function(resultData) { 
//alert(resultData);
