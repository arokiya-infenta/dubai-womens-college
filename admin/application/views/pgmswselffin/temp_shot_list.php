...
<?php  $sl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",1)->where('shotlisted_candidate.created >',$this->syear.'-06-01')
->where('shotlisted_candidate.created <',$this->eyear.'-06-01')->get();
$sl_cou = $sl->num_rows();



$wl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",2)->where('shotlisted_candidate.created >',$this->syear.'-06-01')
->where('shotlisted_candidate.created <',$this->eyear.'-06-01')->get();
$wl_cou = $wl->num_rows();


$rl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",3)->where('shotlisted_candidate.created >',$this->syear.'-06-01')
->where('shotlisted_candidate.created <',$this->eyear.'-06-01')->get();
$rl_cou = $rl->num_rows();
...
?>
					 ...
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>PgMswSelfFin/viewAllotment/1" >Selection List : <?=$sl_cou?> </a>
           </div>
					 <div class="col-md-4 ">
           <a href = "<?=base_url()?>PgMswSelfFin/viewAllotment/2" >Waiting List : <?=$wl_cou?></a>
					 </div>
					 	 <div class="col-md-4 ">
              <a href = "<?=base_url()?>PgMswSelfFin/viewAllotment/3" >Not Eligible List: <?=$rl_cou?></a>
					 </div>
...
<?php  $sl = $this->db->select("*")->from("resrvation_table")->where("rc_main_id",3)->where("rc_cource_id",$this->session->userdata('user')['user_dep_status'])->get();
$sl_cou = $sl->result();

foreach($sl_cou as $v){

    $snt = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)
...
?>
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header"><i class="fa fa-file-text"></i> Shortlist criteria
        ...
        <div class="col-lg-12">
          <div class="card">
           ...
           <div class="col-md-2">
            ...
            <div class="card-body">
            <form method="post" action="<?=base_url()?>PgMswSelfFin/registerAllocation">
...
var saveData = $.ajax({
      type: 'POST',
      url: "<?=base_url()?>PgMswSelfFin/selectAllotmentcount",
      data: {m:m},
      dataType: "text",
...
var saveData = $.ajax({
      type: 'POST',
      url: "<?=base_url()?>PgMswSelfFin/selectAllotmentType",
      data: myKeyVals,
      dataType: "text",
      success: function(resultData) { 
