    ...
    <div class="container-fluid">
	  ...
	  <div class="row">
        <div class="col-lg-12">
	      ...
	      <div class="hide-it" align = "center" ><h4 style="color:#15ca20;"><?php echo $this->session->flashdata('form_err'); ?></h4></div>
			...
			<form action="<?php echo base_url().'pgMswSelfFin/stuCmnty'; ?>" method="post">
			  <div class="row">
			  <div class="col-lg-3">
			   ...
			   <?php $department=array('Community Development','Medical & Psychiatric Social Work','Human Resource Management'); ?>
			   <select class="form-control" name="dpmnt" id="dpmnt">
			   <option value="">Select Specialization</option>
			   <?php $d=1;foreach($department as $dpmnt1) { ?>
			   <option value="<?php echo $d ?>" <?php if($dpmnt==$d){echo 'selected';}?>><?php echo $dpmnt1 ?></option>
			   <?php $d++;} ?>
			   </select>
			   <span style="color:red;"><?php echo form_error('dpmnt'); ?></span>
			   </div>
			   </div>
			   <div class="col-lg-3">
			   <div class="form-group">
	 ...
	 <?php if(isset($stu_list)){?>
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
		  <form action="<?php echo base_url().'pgMswSelfFin/zoom_alloc'; ?>" method="post">
            <div class="card-header">
			   ...
			   <input type="hidden" name="department1" value="<?php echo $dpmnt; ?>" id="department1">
			  ...
			  <input class="form-control input-field" type="text" id="search" placeholder="Type names to search" autocomplete="off" style="width: 300px;">
				 ...
				 <button type="submit" class="btn btn-sm btn-primary" name="submit_link" id="submit_link">Save Link</button>
			     </div>
			     </div>
			   </div>
			</div>
            <div class="card-body">
              ...
              <table id="examplezoomview" class="table table-bordered">
				...
				<?php foreach($stu_list as $slist) { ?>
					...
					$app_no= $this->db->select("*")->from("Applyed_Cources")->Where("user_id",$slist->pr_user_id)->Where("applied_course_id",$dpmnt)->where('main_course_id','3')->get()->row();
                      ...
                      <tr>
                        <td>
						<input type="checkbox" value="<?php echo $slist->pr_user_id; ?>" name="stu_id[]" class="stu_id" style="width:25px;height:25px;"></td>
						<td>
...
$(document).ready(function (){
	...
	$(document).on('change','#dpmnt',function(){
		var dpmnt=$(this).val();
		$('#department1').val(dpmnt);
	});
});
</script>
<script>
var $rows = $('#examplezoomview tr');
