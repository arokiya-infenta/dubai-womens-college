                 ...
                 $dep =array(
                   ...
                   '15'=>"MSW-SF",
                    ...
                    $ent =  $this->db->select("*")->from("online_exam_pannel")->Where("student_id",$value->u_id)->Where("exam_category",$this->Subject)->get();
                      ...
                      ?>
                        <tr>
                        <td><a href="<?=base_url()?>PgMswSelfFin/viewStudent/<?=$value->u_id?>"><?=$value->pr_applicant_name?> </a></td>
...
echo"<p style='color: red'> Not Verified </p>";
                        ...
                        <a   class="btn btn-sm btn-primary" href="<?=base_url()?>PgMswSelfFin/updateStudent/<?=$value->u_id?>"><i class="fa fa-pencil-square-o fa-2x" aria-hidden="true"></i></a>
					...
					<!--	<button type="submit" name="submit_pdf" class="btn btn-sm btn-success"><i class="fa fa-file-pdf-o  fa-2x" aria-hidden="true"></i></button>-->
						<a href="<?=base_url()?>PgMswSelfFin/updateStuContactInfo/<?=$value->u_id?>" class="btn btn-sm btn-success"><i class="fa fa-address-book-o  fa-2x" aria-hidden="true"></i></a>
						<a href="<?=base_url()?>PgMswSelfFin/studentPdf/<?=$value->u_id?>/<?=$value->applied_course_id?>" class="btn btn-sm btn-danger"><i class="fa fa-file-pdf-o  fa-2x" aria-hidden="true"></i></a>
            <a href="<?=base_url()?>PgMswSelfFin/studentCertificate/<?=$value->u_id?>" class="btn btn-sm btn-info"><i class="fa fa-comments fa-2x" aria-hidden="true"></i></a>
            <a href="<?=base_url()?>PgMswSelfFin/UpdateMarkPercentage/<?=$value->u_id?>"  class="btn btn-sm btn-warning"><i class="fa fa-percent fa-2x" aria-hidden="true"></i></a>
            <a href="<?=base_url()?>PgMswSelfFin/SendPersenalEmail/<?=$value->u_id?>"  class="btn btn-sm btn-dark"><i class="fa fa-envelope fa-2x" aria-hidden="true"></i></a>
