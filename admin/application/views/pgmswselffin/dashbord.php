...
$alloc = $this->db->select('*')->from('Applyed_Cources')->where("main_course_id",3)->where("Applyed_Cources.applied_course_id",$this->session->userdata('user')['user_dep_status'])->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get();
...
//print_r($_SESSION);
$appt = $this->db->select('*')->from('Applyed_Cources')->where("main_course_id",3)->get();
...
$data["tot_stu"] = $tot_stu = $this->db->select("online_exam_pannel.*")->from("Applyed_Cources")->join("online_exam_pannel","Applyed_Cources.user_id=online_exam_pannel.student_id","left")->join("new_preview_pg","new_preview_pg.pr_user_id=Applyed_Cources.user_id","left")->Where("Applyed_Cources.applied_course_id",$this->session->userdata('user')['user_dep_status'])->Where("Applyed_Cources.main_course_id",3)->Where("online_exam_pannel.exam_category",$this->Subject)->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get()->num_rows();
				
 $data["eligible"] = $eligible = $this->db->select("online_exam_pannel.*")->from("Applyed_Cources")->join("online_exam_pannel","Applyed_Cources.user_id=online_exam_pannel.student_id","left")->join("new_preview_pg","new_preview_pg.pr_user_id=Applyed_Cources.user_id")->Where("Applyed_Cources.applied_course_id",$this->session->userdata('user')['user_dep_status'])->Where("Applyed_Cources.main_course_id",3)->Where("online_exam_pannel.exam_category",$this->Subject)->Where("online_exam_pannel.total_mark != 0")->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get()->num_rows();
 ...
 $data["not_eligible"] = $tot_stu - $eligible;
  ...
  $data["alloted"] = $this->db
             ...
             ->num_rows(); 
						 ...
						 $tot_sl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("created >",$this->asyear)->where("created <",$this->aeyear)->get();
						 $data["sl_tot"] = $tot_sl->num_rows();
						 
						 
						 
						 $sl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",1)->where("created >",$this->asyear)->where("created <",$this->aeyear)->get();
						 $data["sl_cou"] = $sl->num_rows();
						 
						 
						 
						 $wl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",2)->where("created >",$this->asyear)->where("created <",$this->aeyear)->get();
						 $data["wl_cou"] = $wl->num_rows();
						 
						 
						 $rl = $this->db->select("*")->from("shotlisted_candidate")->where("sl_main_id",3)->where("sl_course_id",$this->session->userdata('user')['user_dep_status'])->where("reservation_status",3)->where("created >",$this->asyear)->where("created <",$this->aeyear)->get();
...
?>
     ...
     <div class="row">
        ...
        <div class="col-lg-6">
          <div class="card">
            <div class="card-header bg-info text-white">Applied MSW SelfFinance Courses</div>
            <div class="card-body">
            <?php
            $tot_aid = 0;
                $aid = $this->db->select('*')->from('Applyed_Cources')->where("main_course_id",3)->where("applied_course_id",1)->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get();
            ...
            ?>
              ...
              <?php
                $aid2 = $this->db->select('*')->from('Applyed_Cources')->where("main_course_id",3)->where("applied_course_id",2)->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get();
            ...
            ?>
              ...
              <?php
                $aid3 = $this->db->select('*')->from('Applyed_Cources')->where("main_course_id",3)->where("applied_course_id",3)->where('Applyed_Cources.applied_date >=',$this->asyear)->where('Applyed_Cources.applied_date <',$this->aeyear)->get();
            ...
            <p class="card-text"> Total MSW SelfFinance  Courses :<b class="ls-view"> <?= $tot_aid?></b></p>
            </div>
          </div>
          </div>
          <div class="col-lg-6">
